...
namespace TestApp.Testers
{
    public class XiTester
    {
        ...
        public XiTester(int padNumber)
        {
            ...
            _ioTesters.Add(new Plugins.IOTester($"XI Pad {padNumber} Axis 1", Library.Providers.XInput, _deviceDescriptor, Library.Bindings.Generic.Axis1).Subscribe());
            // Use Axis 5 (Left Trigger) as one of the test axes, as it reports differently (0..255) than the other axes (-32768..32767)
            _ioTesters.Add(new Plugins.IOTester($"XI Pad {padNumber} Axis 2", Library.Providers.XInput, _deviceDescriptor, Library.Bindings.Generic.Axis5).Subscribe());
