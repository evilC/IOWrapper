...
namespace Core_Interception
{
    ...
    public class Core_Interception : IInputProvider, IOutputProvider, IBindModeProvider
    {
        ...
        private readonly Dictionary<int, IceptKeyboardUpdateHandler> _monitoredKeyboards = new Dictionary<int, IceptKeyboardUpdateHandler>();
        private readonly Dictionary<int, IceptMouseUpdateHandler> _monitoredMice = new Dictionary<int, IceptMouseUpdateHandler>();
            ...
            try
            {
                ...
                if (id < 10)
                {
                    if (!_monitoredKeyboards.ContainsKey(devId))
                    {
                        ...
                        _monitoredKeyboards.Add(devId, new IceptKeyboardUpdateHandler(subReq.DeviceDescriptor, subHandler, BindModeHandler, _deviceLibrary));
                ...
                else
                {
                    ...
                    {
                        var subHandler = new SubscriptionHandler(subReq.DeviceDescriptor, KeyboardEmptyHandler);
                        _monitoredMice.Add(devId, new IceptMouseUpdateHandler(subReq.DeviceDescriptor, subHandler, BindModeHandler, _deviceLibrary));
                    }
        ...
        public void SetDetectionMode(DetectionMode detectionMode, DeviceDescriptor deviceDescriptor, Action<ProviderDescriptor, DeviceDescriptor, BindingReport, int> callback = null)
        {
            ...
            if (id < 10)
            {
                if (detectionMode == DetectionMode.Bind)
                {
                    if (!_monitoredKeyboards.ContainsKey(devId))
                    {
                        ...
                        _monitoredKeyboards.Add(devId, new IceptKeyboardUpdateHandler(deviceDescriptor, subHandler, BindModeHandler, _deviceLibrary));
                    }
            ...
            else
            {
                if (detectionMode == DetectionMode.Bind)
                {
                    ...
                    {
                        var subHandler = new SubscriptionHandler(deviceDescriptor, KeyboardEmptyHandler);
                        _monitoredMice.Add(devId, new IceptMouseUpdateHandler(deviceDescriptor, subHandler, BindModeHandler, _deviceLibrary));
                    }
